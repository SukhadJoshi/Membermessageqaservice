...
MEETING_TERMS = ("meeting","meet "," meet","meet.","meet?","meet!")
...
async def ask(body: AskBody):
    ...
    wants_when = ("when" in q_lower) or ("date" in q_lower) or ("what date" in q_lower) or ("on what date" in q_lower) or ("schedule" in q_lower) or ("planning" in q_lower)
    wants_number = ("how many" in q_lower) or ("number" in q_lower) or ("tickets" in q_lower)
    wants_meeting = ("meeting" in q_lower) or ("meet" in q_lower)
    asked_broadway = "broadway" in q_lower
    asked_ticket_domain = any(t in q_lower for t in TICKET_TERMS)
    asked_travel_domain = any(t in q_lower for t in TRAVEL_TERMS)
    ...
    if wants_number:
        filtered = ranked
        if asked_ticket_domain:
            fb = [mm for mm in filtered if any(term in (mm.get("text") or "").lower() for term in TICKET_TERMS)]
            if fb:
                filtered = fb
        if asked_broadway:
            fb2 = [mm for mm in filtered if "broadway" in (mm.get("text") or "").lower()]
            if fb2:
                filtered = fb2
        for mm in filtered:
    ...
    if wants_when:
        for mm in ranked:
            ...
            if dt:
                if asked_ticket_domain and not any(term in (mm.get("text") or "").lower() for term in TICKET_TERMS):
                    continue
                if wants_meeting and not any(term in (mm.get("text") or "").lower() for term in MEETING_TERMS):
                    continue
